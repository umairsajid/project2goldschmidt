...
public class memsim {
    ...
    public static void main(String args[]) {
        if (args.length != 3) {
            System.out.println("Usage: java memsim [algorithm type] [exit-probability] <process-probability>");
            return;
        }
        configureSimulation(args[0]);
        runSimulation(Integer.valueOf(args[2]), Integer.valueOf(args[1]), false);
    }
    ...
    public static void runSimulation(int processEnterProbability, int processExitProbability, boolean AUTOMATE) {
        ...
        while (true) {//"c".equals(c.readLine())) {
            ...
            pManager.exitProcesses(processExitProbability);
